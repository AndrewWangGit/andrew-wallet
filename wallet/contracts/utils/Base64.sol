...
library Base64 {
    ...
    function encode(string memory _str) internal pure returns (string memory) {
        ...
        for (i = 0; i < padlen; i += 3) {
            ...
            uint c1 = ((uint(uint8(_ms[i])) & 3) << 4) |
                (uint(uint8(_ms[i + 1])) >> 4);
            uint c2 = ((uint(uint8(_ms[i + 1])) & 15) << 2) |
                (uint(uint8(_ms[i + 2])) >> 6);
        ...
        if (res[res.length - 1] == bytes1("=")) {
            bytes memory result = new bytes(res.length - 2);
            for (uint index = 0; index < res.length - 2; index++) {
    ...
    function dencode4(
        ...
        bytes1 b2,
        bytes1 b3
    ) private pure returns (bytes1 a0, bytes1 a1, bytes1 a2) {
